...
{
    if (delegate) {
        [delegate flowCover:self changedSelect:index];
    }
}
...
- (void)handleGesture:(UIPanGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        [self touchAtIndex:self.currentRenderingImageIndex];
    }
...
+ (id)coverFlowViewWithFrame:(CGRect)frame andImages:(NSMutableArray *)rawImages sideImageCount:(int)sideCount sideImageScale:(CGFloat)sideImageScale middleImageScale:(CGFloat)middleImageScale {
    ...
    [flowView setupImages:flowView];

//    [flowView addPageControl];
    
    UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:flowView action:@selector(tapDetected)];
    singleTap.numberOfTapsRequired = 1;
    [flowView setUserInteractionEnabled:YES];
    [flowView addGestureRecognizer:singleTap];
...
-(void)tapDetected{
    if (delegate) {
        [delegate flowCover:self didSelect:self.currentRenderingImageIndex];
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        CATransform3D transformPerspective = CATransform3DIdentity;
                        transformPerspective.m34 = -1.0 / 300.0;
...
- (void)setupImages:(CoverFlowView*)flowView {
